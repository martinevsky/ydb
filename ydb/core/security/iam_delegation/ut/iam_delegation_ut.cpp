...
struct TFixture {
    ...
    TFixture() {
        ...
        Settings.MinServedLifetime = TDuration::Seconds(1);
...
Y_UNIT_TEST_SUITE(IamDelegationSettings) {
    ...
    Y_UNIT_TEST(FromConfigCopiesIamConfig) {
        ...
        UNIT_ASSERT_VALUES_EQUAL(settings.MinServedLifetime, TDuration::Minutes(5));
...
Y_UNIT_TEST_SUITE(IamDelegatedTokenService) {
    ...
    // A token is refreshed at half of its lifetime, while it is still valid.
    Y_UNIT_TEST(RefreshAtHalfOfLifetime) {
        TFixture f;
        f.TokenMock.SetServiceToken("cloud-1", "sa-1", "delegated");
        f.TokenMock.ServiceTokenLifetime = TDuration::Seconds(8); // refreshed after about 4 s, expires after 8 s
        const auto service = f.StartTokenService();

        auto first = f.GetToken(service, KEY);
        UNIT_ASSERT_VALUES_EQUAL(first->Token, "delegated-1");

        // the second mint happens on its own, at half of the lifetime of the first token, before it expires
    ...
    // A cached token below the minimum served lifetime counts as absent: a request is not answered with it but
    // waits for the next mint attempt. The attempt is held by the test, so the reply cannot come from the old token.
    Y_UNIT_TEST(TokenBelowMinimumLifetimeCountsAsAbsent) {
        TFixture f;
        f.TokenMock.SetServiceToken("cloud-1", "sa-1", "delegated");
        f.TokenMock.ServiceTokenLifetime = TDuration::Seconds(10);
        f.Settings.MinServedLifetime = TDuration::Seconds(4); // servable for the first 6 s of a 10 s token
        f.TokenMock.HoldServiceTokenCallsFrom = 2; // every refresh waits for the test
        const auto service = f.StartTokenService();

        const auto first = f.GetToken(service, KEY);
        UNIT_ASSERT_VALUES_EQUAL(first->Token, "delegated-1");
        UNIT_ASSERT_VALUES_EQUAL(first->UsableUntil, first->ExpiresAt - TDuration::Seconds(4));

        // the token is below the minimum from now on (hang guard: the clock makes it inevitable)
        f.WaitUntil([&]() { return TInstant::Now() >= first->UsableUntil; }, "the first token to fall below the minimum");

        f.Runtime->Send(new IEventHandle(service, f.Sender, new TEvIamDelegation::TEvGetToken(KEY)));
        f.TokenMock.ReleaseHeldServiceTokenCalls();
        TAutoPtr<IEventHandle> handle;
        auto* second = f.Runtime->GrabEdgeEvent<TEvIamDelegation::TEvGetTokenResult>(handle);
        UNIT_ASSERT(second);
        UNIT_ASSERT_C(second->IsSuccess(), second->Issues.ToOneLineString());
        UNIT_ASSERT_VALUES_EQUAL(second->Token, "delegated-2"); // never the token below the minimum
    }
