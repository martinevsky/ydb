...
struct TIamDelegationSettings {
    ...
    // A token is served only while at least this much of its lifetime remains (half of the lifetime for
    // tokens shorter than twice this): it covers request processing and clock skew. Tokens are refreshed at
    // half of their lifetime, and at least every MaxTokenCacheLifetime.
    TDuration MinServedLifetime = TDuration::Minutes(5);
