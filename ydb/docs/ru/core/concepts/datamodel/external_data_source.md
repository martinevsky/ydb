...
При использовании {{ ydb-short-name }} в качестве внешнего источника данных в долгосрочных [стриминговых запросах](query_execution/streaming-query.md) стандартная аутентификация по токену становится непрактичной: облачные токены имеют ограниченный срок действия, из-за чего запросы приходится перезапускать каждый раз при истечении токена.
